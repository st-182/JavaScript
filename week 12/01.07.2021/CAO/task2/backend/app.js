...
app.get(`/:user`, (req, res) => {
  if (req.params.user === "email") {
    ...
    res.json(data[req.params.user - 1]);
  }
});
app.get(`/:user/:userdata`, (req, res) => {
  if (
    req.params.user === "search" &&
    req.params.userdata !==
      ("id" || "first_name" || "last_name" || "email" || "gender" || "car")
  ) {
    let searchTerm = req.params.userdata;
    let result = data.filter(
      (item) =>
        item.id === searchTerm ||
        item.first_name === searchTerm ||
        item.last_name === searchTerm ||
        item.email === searchTerm ||
        item.gender === searchTerm ||
        item.car === searchTerm
    );
    res.json(
    ...
    let searchTerm = req.params.userdata;
    let result = data.filter((item) => item.car === searchTerm);
    res.json(result);
  } else {
    res.json(data[req.params.user][req.params.userdata]);
  }
});
// app.get(`/search/:something`, (req, res) => {
//   let searchTerm = req.params.something;
//   let id = [];
//   let first_name = [];
//   let last_name = [];
//   let email = [];
